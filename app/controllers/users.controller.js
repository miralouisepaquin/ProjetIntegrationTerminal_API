...
// Find a single User with an identifiant
exports.findOne = (req, res) => {
    const identifiant = req.params.identifiant;
    Users.findById(identifiant)
      .then(data => {
        if (!data)
          res.status(404).send({ message: "Impossible de trouver le User avec le id : " + identifiant });
        else res.send(data);
      })
      .catch(err => {
        res
          .status(500)
          .send({ message: "Erreur lors de la recherche du User avec le id :=" + identifiant });
      });
};
